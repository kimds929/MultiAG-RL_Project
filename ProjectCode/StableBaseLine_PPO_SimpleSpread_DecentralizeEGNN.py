...
class CustomActorCritic(nn.Module):
    def __init__(self, observation_space, action_space, 
        ...
        self.actor_net = nn.Sequential(
            EGNNLayer(node_attr_dim, coord_dim, msg_dim),
            EGNNLayer(node_attr_dim, coord_dim, msg_dim),
        )
...
checkpoint_path = '/home/kimds929/CodePractice/SimpleSpread_sb3_egnn'
if "ppo_decentalize_egnn_checkpoint.zip" in os.listdir(checkpoint_path):
    model_path = f"{checkpoint_path}/ppo_decentalize_egnn_checkpoint.zip"
...
for i in range(N_ITER):
    ...
    if (i % 5 == 0) or (i == N_ITER-1):
        save_path = f"/home/kimds929/CodePractice/SimpleSpread_sb3_egnn"
        model.save(f"{save_path}/ppo_decentalize_egnn_checkpoint{i}")   # backup
        model.save(f"{save_path}/ppo_decentalize_egnn_checkpoint")   # recent
