...
class GNNLayer(nn.Module):
    def __init__(self, input_dim, msg_dim=32):
        super().__init__()
        self.phi_e = nn.Sequential(nn.Linear(input_dim, msg_dim),
                                   nn.SiLU())
        self.phi_h = nn.Sequential(nn.Linear(4+msg_dim, input_dim),
                                   nn.SiLU())
    
    def forward(self, x):
        hi = torch.mean(x[..., :4], dim=-2)    # (batch, 4)
        mij = self.phi_e(x)     # (batch, node, msg_dim)
        mi = torch.mean(mij, dim=-2)    # (batch, msg_dim)
        # print(hi.shape, mi.shape)
        hi_cat = torch.cat([hi, mi], dim=-1).unsqueeze(-2)    # (batch, input_dim+msg_dim)
        output = self.phi_h(hi_cat)
        return output   # (batch, 1, input_dim)
...
class CustomActorCritic(nn.Module):
    def __init__(self, observation_space, action_space, 
        ...
        input_dim = self.obs_dim[-1]
        
        self.actor_net = nn.Sequential(
            GNNLayer(input_dim, msg_dim),
            GNNLayer(input_dim, msg_dim),
            nn.Linear(input_dim, self.act_dim)
        )
    ...
    def actor_forward(self, obs):
        mean = self.actor_net(obs).squeeze(-2)
    ...
    def critic_forward(self, obs):
        values = self.critic_net(obs).squeeze(-2)
        return values
...
# gnn = GNNLayer(8)
# gnn.forward(torch.rand(5,8)).shape
# gnn.forward(torch.rand(4,5,8)).shape

# cac = CustomActorCritic(torch.rand(5,8), torch.rand(5))
# cac.forward(torch.rand(5,8))
# cac.forward(torch.rand(4,5,8))
# cac.evaluate_actions(torch.rand(5,8), torch.rand(5))
# cac.evaluate_actions(torch.rand(4,5,8), torch.rand(5))
...
env = ss.concat_vec_envs_v1(vec_env, num_vec_envs=4, base_class='stable_baselines3')
...
checkpoint_path = '/home/kimds929/CodePractice/SimpleSpread_sb3_gnn'
if "ppo_decentalize_gnn_checkpoint.zip" in os.listdir(checkpoint_path):
    model_path = f"{checkpoint_path}/ppo_decentalize_gnn_checkpoint.zip"
...
for i in range(N_ITER):
    ...
    if (i % 5 == 0) or (i == N_ITER-1):
        save_path = f"/home/kimds929/CodePractice/SimpleSpread_sb3_gnn"
        model.save(f"{save_path}/ppo_decentalize_gnn_checkpoint{i}")   # backup
        model.save(f"{save_path}/ppo_decentalize_gnn_checkpoint")   # recent
