...
def render_checkpoint(checkpoint_idx=490, n_episodes=1, save_format="gif", output_path=None):
    # 환경 생성 (render_mode="rgb_array" for frame capture)
    env = MultiAgentWrapper(max_steps=25, render_mode="rgb_array")
...
if __name__ == "__main__":
    # GIF 저장 예시
    render_checkpoint(checkpoint_idx=10, n_episodes=1, save_format="gif", output_path="checkpoint_10.gif")
    # MP4 저장 예시 (주석 해제하여 사용)
    # render_checkpoint(checkpoint_idx=490, n_episodes=1, save_format="mp4", output_path="checkpoint_490.mp4")